...
superviseBaseinfoEdit={
	initsuperviseInfoEdit:function () {
    	...
    	$("#saveSuperviseInfoId").click(function(){
    		$("#projectSuperviseEditFormId").submit();
    	});
    	
    	//用户信息验证
    	$("#projectSuperviseEditFormId").validator({
    		valid:function(from){
    			$("#projectSuperviseEditFormId #changeDetails").val(getModifiedField($("#projectSuperviseEditFormId").serializeFormJson(),projectBaseInfoOldVal,projectBaseInfoField));
    			$.ajax({
    				url:basePath+'/taskManage/saveSuperviseInfo',
    				type: 'POST',
    				data: $("#projectSuperviseEditFormId").serialize(),
    				dataType: 'json',
    				success:function(event){
    					$("#closeProjectBaseInfoEditIdEST").click();
...
jQuery(document).ready(function() {
    ...
    $("#projectSuperviseEditFormId #requiredDate").datepicker("setStartDate",new Date());
